  ...
  public class EncounterManager : MonoBehaviour
  {
    ...
    )
    {
      ...
      if (
        ...
        || _currentEncounter.waves.Count == 0
        || _spawnPoints.Count == 0
      )
      {
        Debug.LogWarning(
          "<color=yellow>[Encounter]</color> Нет данных для боя или точек спавна. Авто-зачистка."
        );
        FinishEncounter();
        return;
      }
      ...
      Debug.Log(
        $"<color=red>[Encounter]</color> БОЙ НАЧАЛСЯ! Волн: {_currentEncounter.waves.Count}"
      );
    ...
    {
      while (
        _activeEnemies.Count < _currentEncounter.maxConcurrentEnemies && _enemiesToSpawn.Count > 0
      )
      {
