...
namespace InsideVentura.World
{
    ...
    public class RoomBuilder : MonoBehaviour
    {
        ...
        [SerializeField] private TileBase floorTile; // Пол везде одинаковый
        [SerializeField] private TileBase defaultWallTile; // Стена по умолчанию (для закрытых дверей)

        [Header("ID Configuration")]
        [SerializeField] private List<CellMapping> cellMappings = new List<CellMapping>();
        ...
        public void Build(DungeonRoomData data, bool isCleared, bool hasNorth, bool hasSouth, bool hasEast, bool hasWest)
        {
            if (data == null) return;
            _currentData = data;
            Clear();
        ...
        {
            if (y >= _currentData.height - 1) return DoorDirection.North;
            if (y <= 0) return DoorDirection.South;
            if (x >= _currentData.width - 1) return DoorDirection.East;
