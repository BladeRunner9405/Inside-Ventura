...
public class SwordWeapon : Weapon {
    ...
    [SerializeField] public float baseSpecialDamage = 25f; // Теперь это базовое число, а не ModifiableStat
    [SerializeField] private float lungeDistance = 3f;
    [SerializeField] private float lungeDuration = 0.1f; // Изменил 1f на 0.1f (рывок не должен длиться секунду)

    [Tooltip("Время жизни хитбокса в секундах. 0 = мгновенно")] 
    [SerializeField] private float hitboxActiveTime = 0.1f;

    public override void ExecuteAttack(WeaponInstance instance, Vector2 direction, float normalFinalDamage) 
    {
        var isSpecial = instance.CurrentChainCount == Mathf.RoundToInt(instance.ChainCount.ModifiedValue);
        ...
        var finalDamage = isSpecial ? instance.GetDamageWithCritChance(baseSpecialDamage) : normalFinalDamage;
