...
public class WeaponInstance : ArtifactInstance
{
    ...
    public WeaponInstance(Weapon baseData, PlayerAccessor playerAccessor) : base(baseData, playerAccessor)
    {
        WeaponData = baseData;
    ...
    {
        if (statName == StatName.Damage) return Damage;
        if (statName == StatName.AttackSpeed) return AttackSpeed;
        if (statName == StatName.CritChance) return CritChance;
        if (statName == StatName.ChainCount) return ChainCount;
        return base.GetStat(statName);
    }

    public void TryAttack(Vector2 direction)
    {
        if (Time.time < _lastAttackTime + _currentCooldown) return;
