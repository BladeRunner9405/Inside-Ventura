...
public class AttackAnimator : MonoBehaviour
{
  ...
  [Tooltip(
    "Нужно ли физически поворачивать спрайт по вектору атаки? (Да - для меча, Нет - для волн/вспышек)"
  )]
  [SerializeField]
  private bool rotateToDirection = false;

  [Tooltip("Отзеркаливать ли спрайт, если атака идет влево?")]
  [SerializeField]
  private bool flipOnLeftDirection = true;
